...
$this->registerJsFile(Yii::$app->request->BaseUrl . '/js/invoices2.js');
?>

<div class="invoices-form mt-4">
        ...
        <div class="row pt-2">
            <div class="col-6">
                ...
                $form->field($model, 'fk_doc_type')->widget(Select2::classname(), [
                    'data' => $doc_type,
                    'options' => ['placeholder' => 'Seleccionar Tipo de Documento...', 'onchange' => 'invoicesDoc(this.value)'],
                    'pluginOptions' => [
                        'allowClear' => true
                    ],
                ]);
            ...
            <div class="col-6">
                <?= $form->field($model, 'doc_number')->textInput(['maxlength' => true, 'placeholder' => 'Número de Documento..', 'readonly' => FALSE]) ?>
            </div>
        </div>
        ...
        <table class="table mt-2">
            ...
            <tbody>
                <tr>                    
                    ...
                    <td><input type="text" class="form-control" name="exp_concept" id="expenses_concept" placeholder="Concepto de Gastos.."></td>
                    <td><input type="number" class="form-control" name="exp_price" id="expenses_price" placeholder="Valor.." onkeypress="return isNumberKey(event)"></td>
                    <td>
                        <?php
                        echo Select2::widget([
                            'name' => 'fk_cost_center',
                    ...
                    <td><?= Html::button('+', ['value' => /* Url::to("createcontact") */'', 'class' => 'btn btn-primary', 'onclick' => "addExpense()"]); ?></td>
                </tr>                
            </tbody>
        </table>
        <?php if ($model->isNewRecord) { ?>
            <div id="div_table_exp" class="d-none">
            <?php } else { ?>
                <div id="div_table_exp" class="">
                <?php } ?>        
                <table class="table table-bordered">
                    <thead class="thead-light">
                        <tr>
                            <th scope="col">#</th>
                            <th scope="col">Cuenta de Gastos</th>
                            <th scope="col">Concepto de Gastos</th>
                            <th scope="col">Valor</th>
                            <th scope="col">Centro de Costos a Cargar</th>
                        </tr>
                    </thead>
                    <tbody id="table_exp_body">
                        <?php
                        if (!$model->isNewRecord) {
                            $j = 0;
                            $i = 1;
                            $expenses_input_account = '';
                            $expenses_input_concept = '';
                            $expenses_input_price = '';
                            $expenses_input_cc = '';
                            foreach ($all_expense_list as $expenses) {
                                $expenses_input_account .= $expenses->fk_chart_account . ',';
                                $expenses_input_concept .= $expenses->concept . ',';
                                $expenses_input_price .= $expenses->price . ',';
                                $expenses_input_cc .= $expenses->fk_cost_center . ',';
                                $account = $expenses->fkChartAccount->code . ' - ' . $expenses->fkChartAccount->account;
                                $concept = $expenses->concept;
                                $price = number_format($expenses->price, 2, '.', ',');
                                $expense_cc = $expenses->fkCostCenter->code . ' - ' . $expenses->fkCostCenter->name;
                                echo "<div id='update_exp' class='d-none'><tr id='expense$j'>
                                        <th scope='row' class='row_num'>$i</th>
                                        <td>$account</td>
                                        <td>$concept</td>
                                        <td>$price</td>
                                        <td>$expense_cc<button class='btn btn-link float_right btn_concepts' title='Eliminar' aria-label='Eliminar'data-pjax='0' onclick='deleteExpense($j)'><i class='fas fa-trash-alt'></i></button></td>
                                      </tr></div>";
                                $i++;
                                $j++;
                            }
                            ?>
                        <input type='hidden' name='update_exp_account' id='update_exp_account' value='<?= substr($expenses_input_account, 0, -1); ?>'/>
                        <input type='hidden' name='update_exp_concept' id='update_exp_concept' value='<?= substr($expenses_input_concept, 0, -1); ?>'/>
                        <input type='hidden' name='update_exp_price' id='update_exp_price' value='<?= substr($expenses_input_price, 0, -1); ?>'/>
                        <input type='hidden' name='update_exp_cc' id='update_exp_cc' value='<?= substr($expenses_input_cc, 0, -1); ?>'/>
                        <?php
                    }
                    ?>
                    </tbody>
            ...
            <?php if ($model->isNewRecord) { ?>
                <div id="partial_div" class="form-inline d-none">
                <?php } else { ?>
                            ...
                            <div class="row border-top pt-2">
                                <div class="col-4 border-right">
                                    ...
                                    <div class="form-group" id="submit_div">        
                                        <?= Html::button('Guardar', ['class' => 'btn btn-primary', 'onclick' => "submitForm()", 'disabled' => true, 'id' => 'button_submit']); ?>
                                    </div>
