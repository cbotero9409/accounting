...
var i = 1;
var j = 0;
var sum = 0; //partial price with incomes
...
const table_div_inc = document.getElementById('div_table_inc');
const table_inc_body = document.getElementById('table_inc_body');
//arrays to stock the incomes list data
const account_array = [];
const concept_array = [];
const price_array = [];
const center_array = [];
...
const concept_input = document.getElementById('income_concept');
const price_input = document.getElementById('income_price');
...
const income_account = document.getElementById("update_inc_account");
const income_concept = document.getElementById("update_inc_concept");
const income_price = document.getElementById("update_inc_price");
const income_cc = document.getElementById("update_inc_cc");
...
var l = 0;
var total_taxes = 0;
var total = 0; //incomes + taxes
const total_aux = document.getElementById('total_aux'); //input that shows the total with incomes + taxes
const total_input = document.getElementById('billofsale-total_price'); //input to stock the total price data to be sent
...
//form inputs
const cash_input = document.getElementById('billofsale-cash');
const cash_bank_input = document.getElementById('billofsale-cash_payment_bank');
const cash_bill_input = document.getElementById('billofsale-bill_to_pay');
...
function billsDoc(id) {
    $.ajax({
        ...
        success: function (data) {
            ...
            const doc_input = document.getElementById('billofsale-doc_number');
...
function deptsFunction(dept) {
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            const muni = document.getElementById('billofsale-fk_municipality');
            $("#billofsale-fk_municipality").empty();
...
function addIncome() {
    ...
    if (account_input.value !== '' && center_input.value !== '' && price_input.value !== '' && concept_input.value !== '') {
        ...
        const content = `<tr id='income${j}'>
      <th scope="row" class="row_num">${i}</th>
      <td>${account_text}</td>
      <td>${concept_input.value}</td>
      <td>${price_format.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td>
      <td>${center_text}<button class="btn btn-link float_right btn_concepts" title='Eliminar' aria-label='Eliminar'data-pjax='0' onclick="deleteIncome(${j})"><i class='fas fa-trash-alt'></i></button></td>
    </tr>`;
        table_inc_body.insertAdjacentHTML('beforeend', content);
        //actions after adding income element in arrays and webpage table
...
function deleteIncome(id) {
    ...
    i--;
    //reorganize the number of the rows of the incomes list
...
function checkSubtraction() {
    const min_incomes = incomesTable();
    const min_taxes = taxesTable();
    if (subtract_input.value == '0.00' && min_incomes && min_taxes) {
...
//Submit
function submitForm() {
    //format incomes list data and put in the inputs to be sent
...
if (income_account) {
    ...
    const up_account_array = income_account.value.split(',');
    const up_concept_array = income_concept.value.split(',');
    const up_price_array = income_price.value.split(',');
    const up_cc_array = income_cc.value.split(',');
